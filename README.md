...
* Select anchor points according to the specific state: **"vehicle.cpp" line 163-206**
...
* Transform the above anchor points to the vehicle's local coordinate system and generate spline curve `s`: **"vehicle.cpp" line 364-373**
...
* Convert x,y trajectories to s,d trajectories which are used in collision detect: **"vehicle.cpp" line 406-419**
...
After the distance_ahead is transformed into fuzzy_distance, a parabola cost function is used to calculate the cost.

* The final cost equals to `weights[0] * cost_obstacle_avoidance + weights[1] * cost_traffic_jam`
...
* For now, the proposed method will fail if the vehicle on the adjacent lane suddenly changes lane. One solution will be using more complex prediction models like AMM and trajectory clustering introduced in the lessons.
